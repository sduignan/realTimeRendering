...
const float gaussFilter[25] = float[25](
...
void main() {
	...
	//One texel in texture coords
	float div = 1.0f/shad_resolution;

	//Shadowing and blurring
	for(int i=0; i<5; i++){
		for(int j=0; j<5; j++){
			shadow_factor += eval_shadow(vec4((shad_coord.x+(j-2)*div), (shad_coord.y+(i-2)*div), shad_coord.z, shad_coord.w))*gaussFilter[5*i+j];
		}
	}
