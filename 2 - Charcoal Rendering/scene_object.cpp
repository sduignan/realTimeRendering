...
scene_object::scene_object(char* mesh_name, char* tex_name, char* normal_map_file) {
	normal_mapped = true;
	GLfloat* vp = NULL; // array of vertex points
	GLfloat* vn = NULL; // array of vertex normals
	GLfloat* vt = NULL; // array of texture coordinates
	GLfloat* vtan = NULL; //array of tangents

	assert(loab_obj_file_with_tans(mesh_name, vp, vt, vn, vtan, point_count));

	fill_vao_vbo(point_count, vp, vn, vt, vao, vtan);

	assert(load_texture(tex_name, &tex_handle));
	assert(load_texture(normal_map_file, &normal_handle));
}

scene_object::scene_object(char* mesh_name, char* tex_name, char* normal_map_file, char* noisemap_file, char* texture_array_files[]) {
	normal_mapped = true;
	noise_mapped = true;
	multi_tex = true;
	GLfloat* vp = NULL; // array of vertex points
	...
	assert(load_texture(tex_name, &tex_handle));
	assert(load_texture(normal_map_file, &normal_handle));
	assert(load_texture(noisemap_file, &noisemap_handle));
	//HACK - PLZ FIX - HARDCODED # OF TEXTURES
	assert(load_textures(texture_array_files, &tex_array_handle, 9));
}
...
void scene_object::draw(){
	...
	if (noise_mapped) {
		...
		glBindTexture(GL_TEXTURE_2D, noisemap_handle);
	}
	if (multi_tex) {
		glActiveTexture(GL_TEXTURE3);
		glBindTexture(GL_TEXTURE_2D_ARRAY, tex_array_handle);
	}
...
bool scene_object::load_textures(char* file_names[], GLuint* tex, int layers) {
	glGenTextures(1, tex);
	glBindTexture(GL_TEXTURE_2D_ARRAY, *tex);

	int x, y, n;
	int force_channels = 4;
	unsigned char** image_data = new unsigned char*[layers];

	for (int i = 0; i < layers; i++) {
		image_data[i] = stbi_load(file_names[i], &x, &y, &n, force_channels);
		...
		// Flips image to make it right way up
		for (int row = 0; row < half_height; row++) {
			top = image_data[i] + row * width_in_bytes;
			bottom = image_data[i] + (y - row - 1) * width_in_bytes;
	...
	glTexStorage3D(GL_TEXTURE_2D_ARRAY, 1, GL_RGBA8, x, y, layers);
	for (int i = 0; i < layers; i++) {
		glTexSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, i, x, y, 1, GL_RGBA, GL_UNSIGNED_BYTE, image_data[i]);
	}
...
void scene_object::fill_vao_vbo(int point_count, GLfloat* vp, GLfloat* vn, GLfloat* vt, GLuint& vao, GLfloat* vtan) {
	...
	if (vtan != NULL) {
		...
		glBufferData(GL_ARRAY_BUFFER, 4 * sizeof(float)*point_count, vtan, GL_STATIC_DRAW);
